...
function safeGetElement(id) {
  ...
  const els = {
    sourceType: safeGetElement('sourceType'),
    fileInput: safeGetElement('fileInput'),
    fileInputWrap: safeGetElement('fileInputWrap'),
    ...
    vol: safeGetElement('vol'),
  };
  
  // ---------- Enhanced State ----------
  let audioCtx, analyser, mediaStream, sourceNode, rafId;
  let outputGain = null; // master output gain for file playback
  let bufferSource = null; // for file playback
  let decodedFileBuffer = null; // last chosen file buffer
  ...
  // ---------- Source management (Mic | File) ----------
  
  function updateSourceUI(){
    const type = els.sourceType ? els.sourceType.value : 'mic';
    if (els.fileInputWrap) els.fileInputWrap.style.display = (type==='file') ? '' : 'none';
    // Device select only relevant for mic
    if (els.deviceSelect) els.deviceSelect.parentElement.style.display = (type==='mic') ? '' : 'none';
  }
  
  async function decodeArrayBufferToAudio(buf){
    if (!audioCtx) audioCtx = new (window.AudioContext || window.webkitAudioContext)();
    return await audioCtx.decodeAudioData(buf);
  }
  
  // Removed CML sample loading
  
  async function loadFileBuffer(file){
    const arrayBuffer = await file.arrayBuffer();
    decodedFileBuffer = await decodeArrayBufferToAudio(arrayBuffer);
  }
  
  function connectBufferSource(buffer){
    ...
    // Route to speakers via master gain
    bufferSource.connect(outputGain);
    bufferSource.start();
  }
  
  function saveSettings(){
    localStorage.setItem('spec_fft', els.fftSize.value);
    localStorage.setItem('spec_cmap', els.colormap.value);
    localStorage.setItem('spec_floor', els.floorDb.value);
    localStorage.setItem('spec_overlay', els.overlayBands.checked ? '1':'0');
    if (els.timeBtn) localStorage.setItem('spec_time', String(TIME_WINDOW_SEC));
    // Persist view mode
    localStorage.setItem('spec_view', use3D ? '3d' : '2d');
    if (els.vol) localStorage.setItem('spec_vol', String(els.vol.value));
  }
  function loadSettings(){
    const fft = localStorage.getItem('spec_fft'); if(fft) els.fftSize.value=fft;
    const cm = localStorage.getItem('spec_cmap'); if(cm) els.colormap.value=cm;
    const fl = localStorage.getItem('spec_floor'); if(fl) els.floorDb.value=fl;
    const ov = localStorage.getItem('spec_overlay'); if(ov!==null) els.overlayBands.checked = ov==='1';
    const tw = localStorage.getItem('spec_time'); if (tw) TIME_WINDOW_SEC = parseInt(tw, 10) || 10;
    // Load view mode (default 2D)
    const vm = localStorage.getItem('spec_view');
    if (vm === '3d') use3D = true; else if (vm === '2d') use3D = false;
    const vv = localStorage.getItem('spec_vol'); if (vv!==null && els.vol) els.vol.value = vv;
    updateTimeButton();
  }
  ...
  function makeAnalyser(){
    ...
    const requestedFft = parseInt(els.fftSize.value,10);
    analyser.fftSize = requestedFft;
    const appliedFft = analyser.fftSize;
    if (appliedFft !== requestedFft){
      console.warn(`Requested FFT size ${requestedFft} not supported, using ${appliedFft} instead.`);
      // Reflect actual applied size in the UI
      if (els.fftSize) els.fftSize.value = String(appliedFft);
    }
    // Chrome Music Lab style dynamic range - more vibrant colors
    analyser.minDecibels = -90;
    analyser.maxDecibels = -25;
    analyser.smoothingTimeConstant = 0.0; // Off for test parity
    freqData = new Float32Array(analyser.frequencyBinCount);
    
    // Create Hann window for better frequency resolution
    windowFunction = createHannWindow(analyser.fftSize);
    sampleWindow = new Float32Array(analyser.fftSize);
    hopSize = Math.floor(analyser.fftSize / OVERLAP_FACTOR);
    
    // Initialize time buffer for overlapping
    const bufferSize = Math.ceil(analyser.fftSize / OVERLAP_FACTOR);
    timeBuffer = new Array(bufferSize).fill(null).map(() => new Float32Array(analyser.fftSize));
    bufferIndex = 0;
    
    buildRowToBin();
    // Reconnect current source(s) to the new analyser
    reconnectToAnalyser();
  }
  
  function reconnectToAnalyser(){
    if (!audioCtx || !analyser) return;
    // Safely disconnect existing connections from sources
    try { if (sourceNode) sourceNode.disconnect(); } catch(_) {}
    try { if (bufferSource) bufferSource.disconnect(); } catch(_) {}
    // Mic path: connect only to analyser (no direct output to avoid echo)
    if (sourceNode) {
      sourceNode.connect(analyser);
    }
    // File/sample path: connect to analyser and speakers via master gain
    if (bufferSource) {
      ensureOutputGain();
      bufferSource.connect(analyser);
      bufferSource.connect(outputGain);
  ...
  function ensureOutputGain(){
    if (!outputGain){
      ...
      outputGain.gain.value = els.vol ? parseFloat(els.vol.value) : 1.0;
      outputGain.connect(audioCtx.destination);
    }
  }
  
  function buildRowToBin(){
    const h = els.spec2d.height;
    rowToBin = new Uint16Array(h);
    const nyquist = audioCtx.sampleRate/2;
    const fMin = 20;
    const N = analyser.frequencyBinCount; // bins across 0..nyquist
    const fMax = nyquist * (N - 1) / N; // top bin center
    for(let y=0;y<h;y++){
      const frac = 1 - y/(h-1); // top=1 -> high freq
      // Power-warped log mapping biased to lows
      const adj = Math.pow(frac, FREQ_EXP);
      const f = fMin * Math.pow(fMax/fMin, adj);
      // Map to nearest bin center: bin = round(f / (nyquist/N))
      let bin = Math.round((f * N) / nyquist);
      bin = clamp(bin, 0, N-1);
      rowToBin[y] = bin;
    }
  }
  ...
  function drawFrequencyGrid() {
    ...
    ctx.lineWidth = 1;
    // Draw frequency grid lines - include 30/50 Hz for validation
    const freqLines = [30, 50, 100, 200, 500, 1000, 2000, 5000, 10000, 15000];
    ...
    freqLines.forEach(f => {
      ...
      if (y >= 0 && y < h){
        ...
        const label = f >= 1000 ? `${(f/1000).toFixed(f%1000===0?0:1)} kHz` : `${f} Hz`;
        ctx.fillText(label, 4, y);
      }
    });
  
    // 7-band guides (no background fill): dashed boundary lines + right labels with Hz
    if (els.overlayBands && els.overlayBands.checked) {
      const bands = [
        { name: 'Sub‑bass',   f1: 20,   f2: 60 },
        { name: 'Bass',       f1: 60,   f2: 250 },
        { name: 'Low Mid',    f1: 250,  f2: 500 },
        { name: 'Midrange',   f1: 500,  f2: 2000 },
        { name: 'Upper Mid',  f1: 2000, f2: 4000 },
        { name: 'Presence',   f1: 4000, f2: 6000 },
        { name: 'Brilliance', f1: 6000, f2: 20000 }
      ];
  
      // Unique sorted boundary edges
      const edges = Array.from(new Set(bands.flatMap(b => [b.f1, Math.min(b.f2, fMax)])))
        .filter(f => f >= fMin && f <= fMax)
        .sort((a,b)=>a-b);
  
      // Draw dashed boundary lines across the canvas
      ctx.save();
      ctx.strokeStyle = 'rgba(255,255,255,0.5)';
      ctx.setLineDash([6,4]);
      ctx.lineWidth = 2;
      edges.forEach(f => {
        const y = freqToY(f, fMin, fMax, h) + 0.5;
        ctx.beginPath();
        ctx.moveTo(0, y);
        ctx.lineTo(w, y);
        ctx.stroke();
        // Thick mark at right edge
        ctx.save();
        ctx.setLineDash([]);
        ctx.lineWidth = 3;
        ctx.beginPath();
      ...
      // Labels on the right for each band with Hz range
      ctx.fillStyle = 'rgba(255,255,255,0.85)';
      ctx.textAlign = 'right';
      ctx.textBaseline = 'middle';
      ctx.font = '11px sans-serif';
      bands.forEach(b => {
        const y1 = freqToY(b.f1, fMin, fMax, h);
        const y2 = freqToY(Math.min(b.f2, fMax), fMin, fMax, h);
        const yMid = Math.round((y1 + y2) / 2);
        const rangeLabel = `${b.f1}-${Math.min(b.f2, Math.floor(fMax))} Hz`;
        ctx.fillText(`${b.name} (${rangeLabel})`, w - 14, yMid);
      });
      ctx.restore();
    }
    // Draw time grid - more subtle like Chrome Music Lab
    const timeStep = 1; // every second for better temporal reference
    const pixelsPerSecond = w / TIME_WINDOW_SEC;
    ctx.strokeStyle = 'rgba(255, 255, 255, 0.08)';
    for (let t = 0; t <= TIME_WINDOW_SEC; t += timeStep) {
      const x = w - (t * pixelsPerSecond);
      if (x >= 0) {
        ctx.beginPath();
        ctx.moveTo(x, 0);
        ctx.lineTo(x, h);
        ctx.stroke();
        // Time labels - only every 2 seconds to avoid clutter
        if (t % 2 === 0) ctx.fillText(`-${t}s`, x + 2, h - 5);
      }
    }
  
    // Colored band overlays removed per request (no background fills)
    ctx.restore();
  }
  
  function freqToY(f, fMin, fMax, h){
    const adj = Math.log(f / fMin) / Math.log(fMax / fMin);
    const fracDisplay = Math.pow(clamp(adj, 0, 1), 1 / FREQ_EXP);
    const y = (1 - fracDisplay) * (h - 1);
    return clamp(Math.round(y), 0, h-1);
  }
  ...
  async function start(){
    ...
    if (bufferSource) { try{ bufferSource.stop(); bufferSource.disconnect(); }catch(_){} bufferSource = null; }
    ...
    if (type === 'mic'){
      ...
      if (!decodedFileBuffer && els.fileInput && els.fileInput.files && els.fileInput.files[0]){
        await loadFileBuffer(els.fileInput.files[0]);
      }
      if (!decodedFileBuffer){
        alert('Choose an audio file first.');
        els.startBtn.disabled = false; els.stopBtn.disabled = true; return;
      }
      connectBufferSource(decodedFileBuffer);
    }
  ...
  function stop(){
    ...
    sourceNode = null; bufferSource = null; mediaStream = null; analyser = null; freqData = null; rowToBin = [];
    timeBuffer = [];
  ...
  async function init(){
    try{
      ...
      // Source controls
      if (els.sourceType){
        els.sourceType.addEventListener('change', ()=>{ updateSourceUI(); saveSettings(); });
        updateSourceUI();
      }
      // Removed CML sample UI/events
      if (els.fileInput){
        els.fileInput.addEventListener('change', async ()=>{
          decodedFileBuffer = null; // will be decoded on start
        });
      }
    ...
    if (view3DToggle) {
      ...
      if (els.vol) els.vol.addEventListener('input', ()=>{ if(outputGain) outputGain.gain.value=parseFloat(els.vol.value||'1'); saveSettings(); });
