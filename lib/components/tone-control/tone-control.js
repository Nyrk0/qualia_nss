// lib/components/tone-control/tone-control.js
// Reusable Tone Control Web Component: frequency slider + speaker toggle
// UI-only: emits events and exposes properties; host controls audio.

const TEMPLATE = document.createElement('template');
TEMPLATE.innerHTML = `
  <style>
    :host { display: inline-block; font-family: inherit; color: var(--tone-color, #9aa6b2); }
    .wrap { display: grid; grid-template-columns: 1fr auto; gap: 8px; align-items: center; }
    .slider-cell { grid-column: 1 / -1; }
    .info-cell { display: inline-flex; gap: 8px; align-items: center; justify-self: end; }

    /* Minimalist slider */
    input[type="range"] { width: 100%; max-width: 100%; -webkit-appearance: none; appearance: none; background: transparent; outline: none; }
    ...
    input[type="range"]::-webkit-slider-thumb {
      -webkit-appearance: none; width: 14px; height: 14px; margin-top: -4px; border-radius: 50%; background: currentColor; border: 0; box-shadow: none;
    }
    ...
    input[type="range"]::-moz-range-thumb { width: 14px; height: 14px; border-radius: 50%; background: currentColor; border: 0; box-shadow: none; }

    /* Icon button */
    .btn { padding: 6px 8px; line-height: 1; background: #2a2a2a; color: currentColor; border: 1px solid #3a3a3a; border-radius: 6px; cursor: pointer; }
    .btn.icon svg { display: block; width: 16px; height: 16px; fill: currentColor; stroke: none; }
    .btn.icon.active { box-shadow: 0 0 0 2px rgba(255,255,255,0.08) inset; background: #2a3440; }

    .label { font-variant-numeric: tabular-nums; opacity: 0.9; }
  </style>
  <div class="wrap">
