<?php
// scripts/generate-toc.php
// Scans src/wiki/{Home.md,User-Guide,Developer-Docs} and creates/updates src/wiki/toc-manifest.json
...
// Allow override of wiki root via env or query for different deployments
$wikiRoot = getenv('WIKI_ROOT');
if (!$wikiRoot) {
    $wikiRoot = realpath(join_path(__DIR__, '..', 'src', 'wiki')) ?: '';
}
...
$manifestPath = join_path($wikiRoot, 'toc-manifest.json');
...
// Ensure directory is writable for manifest update
$dirWritable = is_writable($wikiRoot);
$wrote = false;
$writeError = null;
if ($dirWritable) {
    ...
    if ($json === false) {
        ...
        if ($fp === false) {
            ...
            // Obtain an exclusive lock to avoid races under concurrent requests
            if (flock($fp, LOCK_EX)) {
                fwrite($fp, $json);
                fflush($fp);
                flock($fp, LOCK_UN);
