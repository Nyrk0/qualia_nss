...
const sonogramVertexShader = `
  ...
  void main() {
    ...
    float y01 = (dbCal - u_minDb) / (u_maxDb - u_minDb); // 0..1
...
const sonogramFragmentShader = `
  ...
  void main() {
    ...
    // Convert raw texture value (0..1) to dB scale (-100..0) and apply calibration offset
    float db = -100.0 + sample.a * 100.0;
    float dbCal = clamp(db + dbfsOffset, -120.0, 0.0);

    // Normalize the dB value according to the dynamic range uniforms, preventing division by zero
    ...
    float v;
    if (u_enableGate) {
        v = value > 0.1 ? 1.0 : 0.0; // thresholded value
    } else {
        v = value; // raw value
    }
...
class AnalyserView {
    ...
    initAxesBuffers() {
        const gl = this.gl;
        if (!gl) {
            console.error('WebGL context not available in initAxesBuffers');
            return;
        }
        ...
        // Build axes and box vertices after buffers are created
    ...
    _rebuildAxesBox() {
        ...
        if (!this.boxVBO) {
            console.error('boxVBO is null, cannot bind buffer');
            return;
        }
        gl.bindBuffer(gl.ARRAY_BUFFER, this.boxVBO);
        const error = gl.getError();
        if (error !== gl.NO_ERROR) {
            console.error('WebGL error after bindBuffer:', error);
            return;
        }
    ...
    _rebuildTicks() {
        ...
        // This buffer is dynamic, so we bind it here right before updating.
    ...
    createShaderProgram(vertexSource, fragmentSource) {
        ...
        program.u_enableFadeLoc = gl.getUniformLocation(program, 'u_enableFade');
    ...
    drawGL() {
        ...
        gl.uniform1i(shader.u_enableFadeLoc, this.fadeEnabled);
...
// Global variables for module architecture
let spectrogramAnalyserView;
let spectrogramPlayer;
...
function initializeSpectrogram() {
    ...
    if (!startButton || !canvas) {
        console.error('Spectrogram elements not found');
        return;
    }
    ...
    const resizeCanvas = (gl) => {
        ...
        // Don't override CSS sizing - let CSS handle the visual size
        // Only update the backing store size for HiDPI rendering
    ...
    spectrogramAnalyserView = new AnalyserView(canvas);
    ...
    const maxDbValue = document.getElementById('range-max-value');
    ...
    if (vScale) {
        vScale.addEventListener('input', () => {
            ...
            spectrogramAnalyserView.verticalScale = v;
    ...
    if (dbfsOffset) {
        dbfsOffset.addEventListener('input', () => {
            ...
            if (!isNaN(v)) {
                spectrogramAnalyserView.dbfsOffset = v;
    ...
    setVal(xRot, xRotVal, spectrogramAnalyserView.cameraController.xRot);
    setVal(yRot, yRotVal, spectrogramAnalyserView.cameraController.yRot);
    setVal(zRot, zRotVal, spectrogramAnalyserView.cameraController.zRot);
    setVal(yPos, yPosVal, spectrogramAnalyserView.cameraController.yT);
    setVal(zPos, zPosVal, spectrogramAnalyserView.cameraController.zT);
    setVal(vScale, vScaleVal, spectrogramAnalyserView.verticalScale);

    // Source radio handlers
    if (sourceDry) sourceDry.addEventListener('change', () => {
        if (sourceDry.checked) spectrogramPlayer.setMode('dry');
    });
    if (sourceWet) sourceWet.addEventListener('change', () => {
        if (sourceWet.checked) spectrogramPlayer.setMode('wet');
    });

    // Mic DSP handlers
    const pushMicOptions = () => spectrogramPlayer.setMicOptions({
    ...
    if (fadeToggle) fadeToggle.addEventListener('change', e => {
        spectrogramAnalyserView.setFadeEnabled(e.target.checked);
    });
    ...
    function updateColormapSpectrum() {
        ...
        for (let x = 0; x < width; x++) {
            ...
            const hPrime = hue / 60;
            const x1 = c * (1 - Math.abs((hPrime % 2) - 1));
            const m = value - c;
            
            let r, g, b;
            if (hPrime < 1) { r = c; g = x1; b = 0; }
            else if (hPrime < 2) { r = x1; g = c; b = 0; }
            else if (hPrime < 3) { r = 0; g = c; b = x1; }
            else if (hPrime < 4) { r = 0; g = x1; b = c; }
            else if (hPrime < 5) { r = x1; g = 0; b = c; }
            else { r = c; g = 0; b = x1; }
    ...
    if (minDbSlider) minDbSlider.addEventListener('input', updateRange);
    if (maxDbSlider) maxDbSlider.addEventListener('input', updateRange);

    startButton.addEventListener('click', () => {
        // Ensure mode matches selected radio at start
        if (sourceDry && sourceDry.checked) spectrogramPlayer.setMode('dry');
        if (sourceWet && sourceWet.checked) spectrogramPlayer.setMode('wet');
        // Apply initial mic options from UI before starting
        pushMicOptions();
        spectrogramPlayer.live();
        spectrogramAnalyserView.setAnalyserNode(spectrogramPlayer.getAnalyserNode());
        // Update frequency axis to device sample rate
        if (spectrogramPlayer && spectrogramPlayer.context && spectrogramPlayer.context.sampleRate) {
            spectrogramAnalyserView.setSampleRate(spectrogramPlayer.context.sampleRate);
        }
        spectrogramAnalyserView.initByteBuffer();
        spectrogramAnalyserView.start();
        startButton.style.display = 'none';
    });

    // Resize on window resize and container size changes
    window.addEventListener('resize', () => resizeCanvas(spectrogramAnalyserView.gl));
    if ('ResizeObserver' in window) {
        const ro = new ResizeObserver(() => resizeCanvas(spectrogramAnalyserView.gl));
        ro.observe(container);
    }

    canvas.addEventListener('click', event => {
        if (!spectrogramAnalyserView.isRendering) return;
        ...
        const inv_mvp = spectrogramAnalyserView.mvp.inverse();
        let ray_world = [
...
// Export the initialization function for the module loader
window.initializeSpectrogram = initializeSpectrogram;
